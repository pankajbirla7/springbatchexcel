		...
		if (chunk != null && chunk.getItems().size() > 0) {
			for (FileDetails item : chunk.getItems()) {
				if(item.getAgencyFin()!=null) {
					...
					String sql = "INSERT INTO File (AgencyFEIN, Filename,  SumbitDate) VALUES (?, ?, ?)";
					jdbcTemplate.update(sql, Integer.parseInt(item.getAgencyFin()), item.getFileName(),  item.getSubmitDate());
				}else {
					continue;
				}
			}
